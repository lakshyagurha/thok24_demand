...
A 1:1 port of ``supabase/functions/voice-relay/persona.ts``. Composed per
session so the catalog and the customer's own details are current.

The personalisation here arrives as *signed* LiveKit token attributes minted by
the ``voice-token`` Edge Function, so a tampered client cannot claim someone
else's history.
"""
...
def from_attributes(attrs: dict[str, str]) -> Profile:
    """Reads the profile out of the participant's token attributes."""
    import json

    regulars: list[str] = []
    raw = attrs.get("regulars")
    if raw:
        try:
            parsed = json.loads(raw)
            if isinstance(parsed, list):
                regulars = [str(x) for x in parsed if x]
        except ValueError:
            pass

    name = (attrs.get("display_name") or "").strip() or None
    try:
        cart_count = int(attrs.get("cart_count") or 0)
    except ValueError:
        cart_count = 0
    ...
    return Profile(
        name=name,
        regulars=regulars,
        has_address=attrs.get("has_address") == "true",
        cart_count=cart_count,
    )


def build_system_instruction(catalog: str, p: Profile) -> str:
    greet_name = f" The customer's name is {p.name}." if p.name else ""

    regulars = (
        f"\nThey usually buy: {', '.join(p.regulars)}. "
    ...
    address_note = (
        ...
        if p.has_address
            ...
            "\nIMPORTANT: this customer has no saved delivery address. If they want to "
            "order, tell them warmly that they need to add an address first, and do not "
            "call place_order."
        )
    )

    cart_note = (
        f"\nTheir cart already has {p.cart_count} item(s) in it from before."
        if p.cart_count > 0
        else ""
    )

    return f"""You are "Ramu Bhai", the voice of the DxMart kirana shop. You take grocery orders by phone-style conversation.{greet_name}
...
Never place an order that you have not just read back aloud. If place_order returns an error, tell them what it actually says; do not pretend it worked.{address_note}{cart_note}{regulars}
